...
`[d] default settings`, and conditionally `[s] save SVG/PNG`.
...
actions are reachable without opening the control panel.
...
1. **Tap-target pill items** — each item inside the pill becomes a `<button>`
...
2. **Floating action bar** — replace the pill entirely with a compact FAB row
   ...
   the bar is always visible; on fine-pointer it only appears on hover/focus.
...
| `s` | Save SVG/PNG | Only when download action available |
...
- Reload action has been removed from the framework; project-specific reset
  logic should live in the sketch itself.
